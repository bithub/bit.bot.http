...
from bit.bot.http.events import SocketCreatedEvent, SocketLostEvent, ClientAuthEvent
...
def client_auth(evt):
    return evt.socket.transport.write(json.dumps(dict(emit={'helo': ''}
                                                          ,bit=dict(bot=dict(session=(dict(jid='anon@chat.3ca.org.uk/'+evt.session_id
                                                                                           ,persistent=False
                                                                                           )))))))
    evt.socket.transport.write(json.dumps(dict(emit={'helo': ''}
